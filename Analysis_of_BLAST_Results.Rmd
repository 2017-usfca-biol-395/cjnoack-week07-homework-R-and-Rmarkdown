...
```{r Histograms for female and male pident}
...
```{r Tables for top 3 species in females and males}
joined_blast_data_metadata %>%
  filter(sex_s == "female") %>%
  filter(env_material_s == "sebum") %>%
  group_by(sscinames) %>%
...
```{r ggplots for individual patients}
ggplot(joined_blast_data_metadata ,
  aes(x = pident)) +
  geom_histogram() +
facet_grid(sex_s~host_subject_id_s)
```
